...
const DICT = {
  ...
  'nav.survival': { zh: '生存分析', en: 'Survival Analysis' },
  'nav.wgcna': { zh: 'WGCNA', en: 'WGCNA' },
  'nav.lasso': { zh: 'LASSO 建模', en: 'LASSO Biomarker' },
  ...
  // Survival Analysis
  'survival.desc': { zh: '单基因 Kaplan-Meier 生存曲线 + Cox 回归分析', en: 'Single-gene Kaplan-Meier survival curves + Cox regression analysis' },
  'survival.params': { zh: '分析参数', en: 'Parameters' },
  'survival.gene': { zh: '基因名', en: 'Gene Name' },
  'survival.cutoff': { zh: '分组方式', en: 'Cutoff Method' },
  'survival.run': { zh: '运行生存分析', en: 'Run Survival Analysis' },
  'survival.placeholder': { zh: '输入基因名，点击运行查看该基因高/低表达组的生存差异。', en: 'Enter a gene name, click Run to view survival differences between high/low expression groups.' },
  'survival.info': { zh: '按中位数将样本分为高/低表达组，使用 Kaplan-Meier 法绘制生存曲线，Log-rank 检验+Cox 回归评估预后价值。', en: 'Splits samples into high/low groups by median expression. Uses Kaplan-Meier curves, Log-rank test, and Cox regression to assess prognostic value.' },
  'survival.gene_required': { zh: '请输入基因名', en: 'Please enter a gene name' },

  // WGCNA
  'wgcna.desc': { zh: '加权基因共表达网络分析，识别关键基因模块和 Hub 基因', en: 'Weighted Gene Co-expression Network Analysis - identify key gene modules and hub genes' },
  'wgcna.params': { zh: '分析参数', en: 'Parameters' },
  'wgcna.n_genes': { zh: 'Top 变异基因数', en: 'Top Variable Genes' },
  'wgcna.min_module': { zh: '最小模块大小', en: 'Min Module Size' },
  'wgcna.run': { zh: '运行 WGCNA', en: 'Run WGCNA' },
  'wgcna.running': { zh: 'WGCNA 分析中（可能需要1-3分钟）...', en: 'Running WGCNA (may take 1-3 minutes)...' },
  'wgcna.placeholder': { zh: '加载表达数据后，点击运行进行共表达网络分析。需要至少6个样本。', en: 'Load expression data, then click Run. Requires at least 6 samples.' },
  'wgcna.info': { zh: '自动选择软阈值，构建基因共表达网络，识别模块并找出各模块的 Hub 基因。', en: 'Automatically picks soft threshold, builds co-expression network, identifies modules and hub genes per module.' },
  'wgcna.result_title': { zh: 'WGCNA 分析结果', en: 'WGCNA Results' },
  'wgcna.modules': { zh: '基因模块', en: 'Gene Modules' },
  'wgcna.hub_genes': { zh: 'Hub 基因', en: 'Hub Genes' },

  // LASSO
  'lasso.desc': { zh: 'LASSO 回归筛选诊断/预后 Biomarker 基因组合', en: 'LASSO regression for diagnostic/prognostic biomarker gene signature selection' },
  'lasso.params': { zh: '分析参数', en: 'Parameters' },
  'lasso.group1': { zh: '组1关键词', en: 'Group 1 keyword' },
  'lasso.group2': { zh: '组2关键词', en: 'Group 2 keyword' },
  'lasso.n_features': { zh: '最大特征数', en: 'Max Features' },
  'lasso.run': { zh: '运行 LASSO', en: 'Run LASSO' },
  'lasso.placeholder': { zh: '加载数据后，设置分组关键词，运行 LASSO 回归筛选 Biomarker。', en: 'Load data, set group keywords, then run LASSO to select biomarker genes.' },
  'lasso.info': { zh: '使用 L1 正则化的 Logistic 回归（LASSO），从差异基因中筛选最小基因集合用于诊断或预后建模。', en: 'Uses L1-regularized logistic regression (LASSO) to select a minimal gene signature for diagnostic or prognostic modeling.' },
  'lasso.result_title': { zh: 'LASSO 筛选结果', en: 'LASSO Selection Results' },
